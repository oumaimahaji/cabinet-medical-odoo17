...
class PatientPortal(CustomerPortal):
    ...
    def _prepare_home_portal_values(self, counters):
        ...
        if patient:
            ...
            next_rdv = request.env['cabinet.rendezvous'].sudo().search([
            ...
            values['consultation_count'] = request.env['cabinet.consultation'].sudo().search_count([('patient_id', '=', patient.id)])
            ...
            factures = request.env['cabinet.facture'].sudo().search([('patient_id', '=', patient.id), ('state', '=', 'draft')])
            ...
            values['rendezvous_count'] = request.env['cabinet.rendezvous'].sudo().search_count([
                ('patient_id', '=', patient.id),
                ('state', 'in', ['termine', 'en_attente', 'absent'])
            ])
            values['ordonnance_count'] = request.env['cabinet.prescription'].sudo().search_count([('patient_id', '=', patient.id)])
            values['facture_count'] = request.env['cabinet.facture'].sudo().search_count([('patient_id', '=', patient.id)])
            ...
            consultations = request.env['cabinet.consultation'].sudo().search([('patient_id', '=', patient.id)], order='date_consultation desc, id desc', limit=3)
            ...
            prescriptions = request.env['cabinet.prescription'].sudo().search([('patient_id', '=', patient.id)], order='date_prescription desc, id desc', limit=3)
            ...
            factures_recents = request.env['cabinet.facture'].sudo().search([('patient_id', '=', patient.id)], order='date_facture desc, id desc', limit=3)
            ...
            rendezvous = request.env['cabinet.rendezvous'].sudo().search([
                ('patient_id', '=', patient.id),
                ('state', 'in', ['termine', 'en_attente', 'absent'])
    ...
    def portal_my_rendezvous(self, page=1, sortby=None, **kw):
        ...
        rdv_count = Rendezvous.sudo().search_count(domain)
        pager = portal_pager(
            url="/my/rendezvous",
        ...
        rendezvous = Rendezvous.sudo().search(domain, order=order, limit=self._items_per_page, offset=pager['offset'])
    ...
    def portal_my_dossier(self, **kw):
        ...
        """
        values = self._prepare_portal_layout_values()
        patient = request.env['cabinet.patient'].sudo().search(
    ...
    def portal_my_ordonnances(self, page=1, sortby=None, **kw):
        ...
        count = Prescription.sudo().search_count(domain)
        pager = portal_pager(
            url="/my/ordonnances",
            url_args={'sortby': sortby},
            total=count,
        ...
        ordonnances = Prescription.sudo().search(domain, order=order, limit=self._items_per_page, offset=pager['offset'])
